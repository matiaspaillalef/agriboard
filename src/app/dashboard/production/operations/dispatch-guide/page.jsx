...
import CardTableDispatchGuide from "@/components/card/CardTableDispatchGuide";
import { getDataDispatchGuide } from "@/app/api/ProductionApi";
...
const ProductionDispatchGuide = () => {
  const [dataDispatchGuide, setDataDispatchGuide] = useState([]);
  ...
  const fetchData = useCallback(async (companyId) => {
    setIsLoading(true);
    try {
      const data = await  getDataDispatchGuide(companyId);
      const companies = await getDataCompanies();
      
      setDataDispatchGuide(data);
        ...
        <div className="flex w-full flex-col gap-5 mt-3">
          <div className="mt-3 grid grid-cols-1 gap-5 lg:grid-cols-1">
            <div className="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none w-full p-6">
              <CardTableDispatchGuide
                data={dataDispatchGuide}
                thead="Cliente, Fecha, Temporada, Guía"
                downloadBtn={true}
                SearchInput={true}
                actions={true}
                ...
                omitirColumns={["id", "company_id", "boxes", "kg", "quality"]}
              />
            </div>
          </div>
        </div>
...
export default ProductionDispatchGuide;