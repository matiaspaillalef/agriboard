...
import CardTableScaleRegister from "@/components/card/CardTableScaleRegister";
import { getDataGround, getDataScaleRegister } from "@/app/api/ProductionApi";
...
const ProductionScaleRegister = () => {
  const [dataScaleRegister, setDataScaleRegister] = useState([]);
  ...
  const fetchData = useCallback(async (companyId) => {
    setIsLoading(true);
    try {
      const data = await  getDataScaleRegister(companyId);
      const companies = await getDataCompanies();
      
      setDataScaleRegister(data);
        ...
        <div className="flex w-full flex-col gap-5 mt-3">
          <div className="mt-3 grid grid-cols-1 gap-5 lg:grid-cols-1">
            <div className="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none w-full p-6">
              <CardTableScaleRegister
                data={dataScaleRegister}
                thead="Balanza, Calidad, Especie, Variedad, Temporada"
                downloadBtn={true}
                SearchInput={true}
                actions={true}
                ...
                omitirColumns={["id", "company_id", "boxes", "kg_boxes", "date"]}
              />
            </div>
          </div>
        </div>
...
export default ProductionScaleRegister;