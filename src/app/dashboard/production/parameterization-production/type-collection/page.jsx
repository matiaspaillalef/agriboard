...
import CardTableTypeCollection from "@/components/card/CardTableTypeCollection";
import { getDataGround, getDataTypeCollection } from "@/app/api/ProductionApi";
...
const ProductionTypeCollection = () => {
  const [dataTypeCollection, setDataTypeCollection] = useState([]);
  ...
  const fetchData = useCallback(async (companyId) => {
    setIsLoading(true);
    try {
      const data = await  getDataTypeCollection(companyId);
      const companies = await getDataCompanies();
      
      setDataTypeCollection(data);
...
export default ProductionTypeCollection;