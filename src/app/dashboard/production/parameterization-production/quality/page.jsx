...
import CardTableQuality from "@/components/card/CardTableQuality";
import { getDataGround, getDataQuality } from "@/app/api/ProductionApi";
...
const ProductionQuality = () => {
  ...
  const fetchData = useCallback(async (companyId) => {
    setIsLoading(true);
    try {
      const data = await  getDataQuality(companyId);
