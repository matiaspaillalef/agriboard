...
import CardTableExporters from "@/components/card/CardTableExporters";
import { getDataGround, getDataExporters } from "@/app/api/ProductionApi";
...
const ProductionExporters = () => {
  ...
  const fetchData = useCallback(async (companyId) => {
    setIsLoading(true);
    try {
      const data = await  getDataExporters(companyId);
      const companies = await getDataCompanies();
      
      setDataExporters(data);
        ...
        <div className="flex w-full flex-col gap-5 mt-3">
          <div className="mt-3 grid grid-cols-1 gap-5 lg:grid-cols-1">
            <div className="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none w-full p-6">
              <CardTableExporters
                data={dataExporters}
                thead="Rut, Razón Social, Ciudad, Teléfono, Nombre Representante, Estado"
                downloadBtn={true}
                SearchInput={true}
                actions={true}
                ...
                omitirColumns={["id", "company_id", "giro", "state", "address", "web", "legal_representative_rut", "legal_representative_phone", "legal_representative_email"]}
              />
            </div>
          </div>
        </div>
...
export default ProductionExporters;