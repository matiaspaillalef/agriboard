...
import CardTableScale from "@/components/card/CardTableScale";
import { getDataGround, getDataScale } from "@/app/api/ProductionApi";
...
const ProductionScale = () => {
  ...
  const fetchData = useCallback(async (companyId) => {
    setIsLoading(true);
    try {
      const data = await  getDataScale(companyId);
