...
import CardTableDeals from "@/components/card/CardTableDeals";
import { getDataGround, getDataDeals } from "@/app/api/ProductionApi";
...
const ProductionDeals = () => {
  ...
  const fetchData = useCallback(async (companyId) => {
    setIsLoading(true);
    try {
      const data = await  getDataDeals(companyId);
        ...
        <div className="flex w-full flex-col gap-5 mt-3">
          <div className="mt-3 grid grid-cols-1 gap-5 lg:grid-cols-1">
            <div className="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none w-full p-6">
              <CardTableDeals
                data={dataDeals}
                thead="Nombre, Conf. Cosecha, Precio X Kg., Estado"
                downloadBtn={true}
                SearchInput={true}
                actions={true}
...
export default ProductionDeals;