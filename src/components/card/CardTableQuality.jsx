...
const CardTableQuality = ({
  ...
  const onUpdateItem = async (data) => {
    try {
      ...
      const updateItemApi = await updateQuality(data);
      const dataNew = await getDataQuality(companyID);
  ...
  const handlerRemove = async () => {
    ...
    try {
      //if (userConfirmed) {
      const deleteItem = await deleteQuality(id);
  ...
  const handlerClone = async () => {
    ...
    try {
      const cloneItem = await createQuality(itemToClone);
      const dataNew = await getDataQuality(companyID);
  ...
  const onSubmitForm = async (data) => {
    try {
      ...
      const createItem = await createQuality(data);
      const dataNew = await getDataQuality(companyID);
...
export default CardTableQuality;
