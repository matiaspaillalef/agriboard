...
import {
  ...
  deleteTypeCollection,
} from "@/app/api/ProductionApi";

const CardTableTypeCollection = ({
  data,
  thead,
  columnsClasses = [],
  omitirColumns = [],
  ...
  const onUpdateItem = async (data) => {
    try {
      ...
      const updateItemApi = await updateTypeCollection(updatedData);
      const dataNew = await getDataTypeCollection(companyID);
  ...
  const handlerRemove = async () => {
    ...
    try {
      //if (userConfirmed) {
      const deleteItem = await deleteTypeCollection(id);
  ...
  const handlerClone = async () => {
    ...
    try {
      const cloneItem = await createTypeCollection(itemToClone);
      const dataNew = await getDataTypeCollection(companyID);
  ...
  const onSubmitForm = async (data) => {
    try {
      ...
      const createItem = await createTypeCollection(transformedData);
      const dataNew = await getDataTypeCollection(companyID);
          ...
          >
            <>
              <h2 className="text-center mb-7 text-xl mt-5 dark:text-white">
                ¿Seguro que desea {openAlert ? "eliminar" : "clonar"} la recolección{" "}
...
export default CardTableTypeCollection;