...
import {
  ...
  getDataQuality,
  getDataSeasons,
  getDataScaleRegister,
  updateScaleRegister,
  createScaleRegister,
  deleteScaleRegister,
} from "@/app/api/ProductionApi";

const CardTableScaleRegister = ({
  data,
  thead,
  columnsClasses = [],
  omitirColumns = [],
  ...
  const onUpdateItem = async (data) => {
    try {
      ...
      const updateItemApi = await updateScaleRegister(updatedData);
      const dataNew = await getDataScaleRegister(companyID);
  ...
  const handlerRemove = async () => {
    ...
    try {
      //if (userConfirmed) {
      const deleteItem = await deleteScaleRegister(id);
  ...
  const handlerClone = async () => {
    ...
    try {
      const cloneItem = await createScaleRegister(itemToClone);
      const dataNew = await getDataScaleRegister(companyID);
  ...
  const onSubmitForm = async (data) => {
    ...
    try {
      ...
      const createItem = await createScaleRegister(transformedData);
      const dataNew = await getDataScaleRegister(companyID);
              ...
              {!openShowUser ? (
                ...
                <div className="flex flex-col gap-3">
                  <p className="text-md font-semibold text-gray-800 dark:text-white">
                    <strong>
                      {getNameByKey("scale", selectedItem.scale, dataMap)}
                    </strong>
                  </p>
...
export default CardTableScaleRegister;
