...
const CardTableScale = ({
  ...
  const onUpdateItem = async (data) => {
    ...
    try {
      ...
      const updateItemApi = await updateScale(data);
      const dataNew = await getDataScale(companyID);
  ...
  const handleCloneAlert = (index, name, location, status, company_id) => {
    ...
    setItemToClone({ name, location, status, company_id });
  ...
  const handleCloseAlertClone = () => {
    setOpenAlertClone(false);
    setOpenAlert(false);
    setItemToDelete({ index: null, name: "", location: "", status: "" });
  };
  ...
  const handlerRemove = async () => {
    ...
    try {
      //if (userConfirmed) {
      const deleteItem = await deleteScale(id);
  ...
  const onSubmitForm = async (data) => {
    ...
    try {
      ...
      const createItem = await createScale(data);
      const dataNew = await getDataScale(companyID);
            ...
            <div className="buttonsActions mb-3 flex gap-2 w-full flex-col md:w-auto md:flex-row md:gap-5">
              {Array.isArray(initialData) &&
                ...
                downloadBtn && (
                  <ExportarExcel
                    data={initialData}
                    filename="Tipos de Balanza"
                    sheetname="Tipos de Balanza"
          ...
          >
            <>
              <h2 className="text-center mb-7 text-xl mt-5 dark:text-white">
                ¿Seguro que desea {openAlert ? "eliminar" : "clonar"} la Balanza{" "}
